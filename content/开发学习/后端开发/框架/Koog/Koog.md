...
> ## **策略图组件**
>
> ### _节点_：节点是策略图的构建块。每个节点代表一个特定操作。Koog 框架提供了预定义节点，用户也可以使用`node`函数创建自定义节点
>
> ### _边_：边连接节点并定义策略图中的操作流。边通过 `edge` 函数和 `forwardTo` 中缀函数创建：
>
> ```kotlin
> edge(sourceNode forwardTo targetNode)
> ```
>
> ### _条件_：条件决定了何时跟随策略图中的特定边
>
> 下列罗列一些比较常见的条件：
>

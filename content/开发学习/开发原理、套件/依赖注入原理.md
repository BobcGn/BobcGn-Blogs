...
class OrderService {
  ...
  set logger(logger: ILogger) { this._logger = logger; }
...
class IoCContainer {
  ...
  resolve<T>(token: Token): T {
    ...
    const resolvedParams = paramTypes.map(paramToken =>
      this.resolve(paramToken)  // ← 递归调用，这是核心！
    );
...
> 这是一个常见的反模式：
> ```typescript
> class A { constructor(private b: B) {} }   // A 依赖 B
> class B { constructor(private a: A) {} }   // B 依赖 A ← 循环！
> ```

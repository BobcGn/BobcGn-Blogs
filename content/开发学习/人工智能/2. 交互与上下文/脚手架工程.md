...
## 1.4 从"给 Agent 加插件"到"组装 Agent 运行时"

> [!info] 架构思考：Harness 的抽象跃迁
>
> 传统插件系统解决的问题通常是："Agent 可以调用哪些额外工具？" 例如 MCP Server、Skill、浏览器、数据库、Shell。它们扩展的是 Agent 的能力边界。
>
> 更进一步的 Harness 工程要解决的问题是："Agent 自身如何运行？" 也就是循环（Loop）、会话（Session）、上下文策略（Context Policy）、记忆（Memory）、沙盒（Sandbox）、文件系统（Filesystem）、编排（Orchestration）、UI 是否也能成为可替换、可组合、可观测的组件。

这两种抽象的差别非常大：

| 抽象层级 | 典型问题 | 工程对象 |
| :--- | :--- | :--- |
| 工具 / MCP | Agent 能做什么？ | 外部能力与系统调用 |
| 技能（Skill） | Agent 知道怎么做什么？ | 任务方法、领域经验、工作流片段 |
| 上下文策略（Context Policy） | Agent 本轮应该看到什么？ | 上下文选择、压缩、淘汰、缓存 |
| 循环（Loop） | Agent 如何决定下一步做什么？ | ReAct、Plan-and-Execute、CodeAct、自定义控制算法 |
| 脚手架（Harness） | Agent 如何被组织、约束和恢复？ | 状态、权限、沙盒、日志、回放、编排 |

如果只有 Tool / MCP 插件化，Agent 仍然是"一个框架写死的程序 + 一些外部扩展"。如果 Loop、Context、Session、Sandbox 都能插件化，Agent 就更接近：

```
Agent = 一组插件按照某种拓扑组合出来的运行时
```

这也是为什么脚手架工程越来越像系统软件：模型像 CPU，上下文窗口像内存，外部记忆像虚拟内存，工具调用像系统调用，沙盒像进程隔离，Loop 像调度器，执行轨迹像调试器与审计日志。

> [!important] 架构重点：能力优先，而不是流程优先
>
> 固定业务流程适合**流程优先（Workflow-first）**：先设计 A → B → C 的节点，再把模型塞进节点。但通用 Agent 更适合**能力优先（Capability-first）**：先定义 Agent 拥有哪些能力、每种能力的权限边界、运行预算、失败策略和可观测信号，再由 Loop 在运行时动态组合这些能力。
>
> 对通用 Agent 来说，过早写死 Workflow 会限制模型的探索空间；没有 Capability 边界又会让模型的副作用失控。Harness 的价值就在于同时提供**自由探索的空间**和**不可越过的工程边界**。
...
## 2.4 Loop 插件化：把 Agent 的控制算法变成一等公民

### 2.4.1 为什么 Loop 不应该藏在框架内部

很多 Agent 框架把核心循环写死在运行时里：
...
- Session Checkpoint
- Termination 判断
- Trajectory 记录与 Replay

当这些逻辑全部堆进一个 `AgentRuntime.run()` 时，Loop 就不再是业务代码里的 while 循环，而是整个 Agent 的**控制算法**。如果控制算法不可替换，Agent 的行为边界就会被框架作者固定。

> [!important] 架构重点：Loop 是 Harness 的调度器
>
> Tool 决定 Agent 能做什么，Context 决定 Agent 看到什么，Model 决定 Agent 如何推理，而 Loop 决定 Agent **下一步怎样行动**。因此 Loop 必须像 Model、Tool、Memory 一样成为可配置、可测试、可替换的一等组件。

### 2.4.2 三种典型 Loop 的适用边界

| Loop 类型 | 控制方式 | 适用场景 | 主要风险 |
| :--- | :--- | :--- | :--- |
| **ReAct Loop** | Reason → Act → Observe 循环 | 探索式任务、交互式排障、代码库调查 | 步数膨胀、上下文污染、终止条件不稳定 |
| **Plan-and-Execute Loop** | 先规划任务，再按计划执行与验证 | 长任务、交付型 Coding Agent、迁移重构 | 初始计划错误会系统性放大，需要中途重规划 |
| **CodeAct Loop** | 模型生成代码，由沙盒执行，再回传结果 | 数据分析、批处理、结构化文件操作 | 沙盒与权限必须严格，避免代码副作用外泄 |

```mermaid
flowchart TB
    Input["用户任务"] --> Router{"Loop Router<br/>任务类型 + 风险等级 + 预算"}

    Router -->|"开放探索"| ReAct["ReAct Loop<br/>逐步观察与行动"]
    Router -->|"长任务交付"| Plan["计划-执行 Loop<br/>计划 / 执行 / 验证"]
    Router -->|"结构化计算"| CodeAct["CodeAct Loop<br/>生成代码 / 沙盒执行 / 汇总"]

    ReAct --> Trace["统一 Trajectory"]
    Plan --> Trace
    CodeAct --> Trace

    Trace --> Eval["Replay / Fork / Eval"]

    style Router fill:#fff3e0,stroke:#f57c00
    style Trace fill:#e1f5fe,stroke:#0288d1,stroke-width:2px
```

> [!tip] 最佳实践：Loop Router 不等于让模型自由选 Loop
>
> Loop 的选择应由 Harness 的确定性策略主导：任务风险、工具副作用、预算、是否需要审批、是否存在可验证输出。LLM 可以提供分类建议，但最终路由应由规则和策略裁决。

### 2.4.3 Loop 插件的最小接口

一个可替换的 Loop 插件，至少需要暴露四类接口：

```python
"""
Loop 插件的最小抽象：让 Agent 的控制算法可替换、可观测、可评测。
"""
from dataclasses import dataclass
...
@dataclass
class LoopDecision:
    action: Literal["call_model", "call_tool", "spawn_subagent", "ask_approval", "finish", "fail"]
    reason: str
    payload: dict
...
class AgentLoop(Protocol):
    ...
    def initialize(self, task: "Task", ctx: "AgentContext") -> "LoopState":
        """初始化 Loop 状态，生成第一个 checkpoint。"""
        ...

    def decide(self, state: "LoopState", observation: "Observation") -> LoopDecision:
        """根据当前状态和环境观察，决定下一步动作。"""
        ...

    def apply(self, state: "LoopState", event: "LoopEvent") -> "LoopState":
        """将模型响应、工具结果、审批结果等事件写回状态。"""
        ...
...
这个接口背后的关键不是代码形式，而是职责边界：

- Loop 插件负责决策节奏，不直接绕过权限系统
- Tool 调用必须经过 Tool Registry
- Context 构造必须经过 Context Assembler
- 状态变化必须写入 Session / Checkpoint
- 每个 decision 必须进入 Trajectory，便于 Replay 和 Eval

### 2.4.4 插件化带来的复杂度成本

> [!warning] 工程避坑："万物皆插件"不等于"万物都应该插件化"
>
> 当 Model、Loop、Memory、Context、Tool、Sandbox、Orchestrator 全部可替换时，系统表达力会显著提升，但故障定位也会变难。一个 Bad Case 可能来自模型输出、Prompt、上下文压缩、Loop 决策、工具错误、记忆污染、沙盒差异，或者插件之间的交互。

因此，完全插件化的 Harness 必须同时建设三件事：

| 基础设施 | 作用 | 没有它的后果 |
| :--- | :--- | :--- |
| **可观测性（Observability）** | 记录每个插件的输入、输出、耗时、状态变化 | 只能看到最终失败，看不到失败传播路径 |
| **重放 / 分叉（Replay / Fork）** | 从任意 checkpoint 重放或分叉执行 | 无法复现线上 Bad Case，也无法比较修复效果 |
| **评估（Evaluation）** | 对 Model / Loop / Context / Tool 组合做回归评测 | 插件替换靠感觉，版本升级不可控 |

> [!quote] 核心观点
> 插件化 Harness 的成败不取决于插件数量，而取决于它能否把插件组合后的行为变成可观测、可复现、可评测的工程事实。
...
> - **运行时内核视角**：高级 Harness 不只是给 Agent 加工具，而是把 Loop、上下文策略、Session、Memory、Sandbox、Orchestration 都变成可组合的运行时组件。
> - **能力优先**：通用 Agent 不应过早写死 Workflow，而应先定义能力、权限、预算、失败策略和可观测信号，再由 Loop 动态组合能力。
> - **Loop 插件化**：Loop 是 Agent 的控制算法，应像 Model 和 Tool 一样可替换、可观测、可评测；插件化必须配套可观测性、重放 / 分叉和评估体系。
